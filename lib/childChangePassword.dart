...
class ChildChangePassword extends StatefulWidget {
  final String child_id;
  const ChildChangePassword({Key key, this.child_id}) : super(key: key);

  @override
  _ChildChangePasswordState createState() => _ChildChangePasswordState();
}

class _ChildChangePasswordState extends State<ChildChangePassword> {
  @override
  void dispose() {
    super.dispose();
    pass1.dispose();
  ...
  Future ChangePass(String pass1, String pass2, String child_id) async {
    final response = await http.post(
        "http://10.0.2.2/api/noom_app/ChildChangePass.php",
        body: {"pass1": pass1, "pass2": pass2, "child_id": child_id});
    ...
    if (reqStatus[0]['done'] == "true") {
      ...
      Future.delayed(d, () {
        Navigator.pushReplacementNamed(context, '/childHomeScreen');
      });
    } else {
      snackBarScafflod.currentState.showSnackBar(SnackBar(
                                  ...
                                  onPressed: () {
                                    if (formKey.currentState.validate()) {
                                      ...
                                      if (pa1 != pa2) {
                                        ...
                                        String child_id =
                                            widget.child_id.toString();

                                        ChangePass(
                                            pass1.text, pass2.text, child_id);
                                      }
                                    }
                                  },
                                  color: Colors.lightBlue,
